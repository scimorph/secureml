...
class _DataConverter:
    """Handles conversion between various data formats and pandas DataFrame."""

    def __init__(self, data: Any, columns: Optional[List[str]] = None):
    ...
    def to_dataframe(self) -> pd.DataFrame:
        ...
        if isinstance(data, np.ndarray):
            if self.columns and len(self.columns) != data.shape[1]:
                raise ValueError(
                    f"Provided {len(self.columns)} column names for a NumPy array with {data.shape[1]} columns."
                )
            return pd.DataFrame(data, columns=self.columns)

        # 5. List of Lists/Tuples (row-oriented)
        if isinstance(data, list) and data and isinstance(data[0], (list, tuple)):
            if self.columns and len(self.columns) != len(data[0]):
                raise ValueError(
                    f"Provided {len(self.columns)} column names for data with {len(data[0])} columns."
                )
            return pd.DataFrame(data, columns=self.columns)

        # 6. File Path or String (CSV/JSON)
        if isinstance(data, str):
            if data.lower().endswith(".csv"):
                return pd.read_csv(data)
            if data.lower().endswith(".json"):
                # Assuming records-oriented JSON, can be made more robust
                return pd.read_json(data, orient="records")
            # Attempt to parse as a string buffer
            try:
                return pd.read_csv(io.StringIO(data))
            except Exception:
                raise ValueError(
                    "String input could not be parsed as CSV. If it's a file, ensure it has a .csv or .json extension."
                )

        # 7. Polars DataFrame
        if self.pl and isinstance(data, self.pl.DataFrame):
            # Store the specific Polars type for conversion back
            self.original_type = self.pl.DataFrame
            return data.to_pandas()

        # 8. PyArrow Table
        if self.pa and isinstance(data, self.pa.Table):
            self.original_type = self.pa.Table
            return data.to_pandas()

        raise TypeError(
            f"Unsupported data type for anonymization: {self.original_type.__name__}"
        )

    def from_dataframe(self, df: pd.DataFrame) -> Any:
        """Converts a pandas DataFrame back to the original input format."""

        # Check original type to determine conversion
        # Use a mapping for cleaner logic
        conversion_map = {
            pd.DataFrame: lambda d: d,
            list: lambda d: d.to_dict("records"),
            dict: lambda d: d.to_dict("list"),
            np.ndarray: lambda d: d.to_numpy(),
            str: lambda d: d,  # Return DataFrame for file/string inputs
        }

        # Add optional dependency conversions
        if self.pl:
            conversion_map[self.pl.DataFrame] = lambda d: self.pl.from_pandas(d)
        if self.pa:
            conversion_map[self.pa.Table] = lambda d: self.pa.Table.from_pandas(d)

        # Find the appropriate converter function
        # We check with issubclass for robustness (e.g., list of dicts vs. list of lists)
        for type_class, func in conversion_map.items():
            if self.original_type is type_class or (
                isinstance(self.original_data, type_class)
            ):
                return func(df)

        # Default fallback for list of lists/tuples
        if isinstance(self.original_data, list):
            return df.to_records(index=False).tolist()

        # If no specific conversion is found, return the DataFrame
        return df
...
def anonymize(
    ...
    columns: Optional[List[str]] = None,  # New argument for array-like data
    **kwargs: Any,
) -> Any:
    """
    ...
    - pandas DataFrame
    - list of dictionaries (e.g., [{'col1': 1}, {'col1': 2}])
    - dictionary of lists (e.g., {'col1': [1, 2]})
    - NumPy array (requires 'columns' argument)
    - File path to a .csv or .json file
    - Polars DataFrame (if installed)
    - PyArrow Table (if installed)
    ...
    Args:
        ...
        columns: List of column names, required for inputs like NumPy arrays
    ...
    Returns:
        ...
        For file path inputs, returns a pandas DataFrame.
    ...
    """
    # 1. Use the converter to handle input
    converter = _DataConverter(data, columns)
    df = converter.to_dataframe()

    # If no sensitive columns are specified, try to identify them automatically
    if sensitive_columns is None:
        sensitive_columns = _identify_sensitive_columns(df)
    ...
    # 2. Apply the selected anonymization method (core logic is unchanged)
    if method == "k-anonymity":
        result_df = _apply_k_anonymity(df, sensitive_columns, k, **kwargs)
    elif method == "pseudonymization":
        result_df = _apply_pseudonymization(df, sensitive_columns, **kwargs)
    elif method == "data-masking":
        result_df = _apply_data_masking(df, sensitive_columns, **kwargs)
    elif method == "generalization":
        result_df = _apply_generalization(df, sensitive_columns, **kwargs)
    else:
        raise ValueError(f"Unsupported anonymization method: {method}")

    # 3. Convert the result back to the original format
    return converter.from_dataframe(result_df)
...
def _identify_sensitive_columns(data: pd.DataFrame) -> List[str]:
    ...
    for col in data.columns:
        ...
        if data[col].dtype == "object":
            ...
            sample = data[col].dropna().astype(str).sample(sample_size, replace=True)
    ...
    **kwargs: Any,
    ...
    if quasi_identifier_columns is None:
        quasi_identifier_columns = [
            col for col in anonymized_data.columns if col not in sensitive_columns
        ]
        ...
        for col in categorical_qi:
            if col in categorical_generalization_levels:
                generalized_data[col] = (
                    generalized_data[col]
                    .map(categorical_generalization_levels[col])
                    .fillna("[UNKNOWN]")
                )
        ...
        for col in numerical_qi:
            ...
            if level > 0:
                ...
                if numeric_generalization_strategy == "equal_width":
                    ...
                    bin_width = (col_max - col_min) / (
                        10 ** (max_numeric_levels - level)
                    )

                    # Apply binning
                    generalized_data[col] = (
                        np.floor((generalized_data[col] - col_min) / bin_width)
                        * bin_width
                        + col_min
                    )
        ...
        if len(small_class_records) / len(generalized_data) > suppression_threshold:
            ...
            if qi_small_class_counts:
                next_qi_to_generalize = max(
                    qi_small_class_counts.items(), key=lambda x: x[1]
                )[0]
                ...
                if next_qi_to_generalize in numerical_qi:
                    ...
                    value_counts = anonymized_data[col].value_counts()
                    least_frequent = value_counts.tail(
                        max(2, int(len(value_counts) * 0.2))
                    ).index.tolist()
    ...
    for col in numerical_qi:
        ...
        if level > 0:
            ...
            if numeric_generalization_strategy == "equal_width":
                ...
                result_data[col] = (
                    np.floor((result_data[col] - col_min) / bin_width) * bin_width
                    + col_min
                )
            elif numeric_generalization_strategy == "equal_frequency":
                n_bins = 10 ** (max_numeric_levels - level)
                bins = np.unique(
                    np.percentile(
                        result_data[col].dropna(), np.linspace(0, 100, n_bins + 1)
                    )
                )
                bin_labels = [(bins[i] + bins[i + 1]) / 2 for i in range(len(bins) - 1)]

                result_data[col] = pd.cut(
                    result_data[col], bins=bins, labels=bin_labels, include_lowest=True
                )
    ...
    **kwargs: Any,
    ...
    def _preserve_format(
        ...
        if not format_pattern:
            ...
            return re.sub(
                format_pattern, lambda m: pseudonym[: len(m.group())], original
            )
    ...
    def _fpe_pseudonym(
        ...
        if key is None:
            ...
            key = default_key_manager.get_encryption_key(
                key_name=master_key_name, encoding="bytes"
            )
    ...
    for col in sensitive_columns:
        ...
        if strategy == "hash":
            pseudonymized_data[col] = pseudonymized_data[col].map(
                lambda x: _preserve_format(x, _generate_hash(str(x)), format_pattern)
                if pd.notna(x)
                else x
            )
        elif strategy == "fpe":
            pseudonymized_data[col] = pseudonymized_data[col].map(
                lambda x: _fpe_pseudonym(str(x), format_pattern) if pd.notna(x) else x
            )
        elif strategy == "deterministic":
            pseudonymized_data[col] = pseudonymized_data[col].map(
                lambda x: _deterministic_pseudonym(str(x), salt, format_pattern)
                if pd.notna(x)
                else x
            )
        else:  # custom
    ...
    **kwargs: Any,
                ...
                if c.isalpha():
                    if c.isupper():
                        replacement = random.choice(
                            [char for char in char_set if char.isupper()]
                        )
                    else:
                        replacement = random.choice(
                            [char for char in char_set if char.islower()]
                        )
                elif c.isdigit():
                    replacement = random.choice(
                        [char for char in char_set if char.isdigit()]
                    )
                else:
    ...
    for col in sensitive_columns:
        ...
        if preserve_format:
            ...
            col_config = masking_rules.get(
                col,
                default_format_preserving_rules.get(
                    col.lower(), {"strategy": default_strategy}
                ),
            )
        else:
        ...
        if strategy == "character":
            ...
            masked_data[col] = masked_data[col].apply(
                lambda x: _regex_mask(x, pattern, replacement) if pd.notna(x) else x
            )

        elif strategy == "random":
            char_set = col_config.get(
                "char_set",
            ...
            if is_numeric and col_config.get(
            ...
            masked_data[col] = masked_data[col].apply(
                lambda x: _redact_value(x, redaction_text) if pd.notna(x) else x
            )
    ...
    **kwargs: Any,
    ...
    def _apply_range_generalization(
        ...
        def generalize_value(value):
            ...
            lower_bound = (
                math.floor((value - min_bound) / range_size) * range_size + min_bound
            )
    ...
    def _apply_binning(
        ...
        if strategy == "equal_width":
            bins = np.linspace(non_na_values.min(), non_na_values.max(), num_bins + 1)
            binned = pd.cut(series, bins=bins, labels=labels, include_lowest=True)
        elif strategy == "equal_frequency":
            bins = np.percentile(non_na_values, np.linspace(0, 100, num_bins + 1))
            # Ensure unique bin edges
            bins = np.unique(bins)
            if len(bins) <= 1:  # Handle case with all same values
                return pd.Series(
                    str(series.iloc[0]) if len(series) > 0 else "", index=series.index
                )
    ...
    def _apply_topk_generalization(
        ...
        if min_frequency is not None:
            ...
            frequent_values = value_counts[
                value_counts >= min_frequency * len(series)
            ].index.tolist()
